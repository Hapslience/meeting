...
//------------------------------------------
function createMeetingInfo(req,res){
	console.log("-------------------------------");
	console.log("action:createMeetingInfo");
	var userName=req.session.user.userName;
	if(typeof req.body.meetingName=='undefined'){
		var errmsg="请输入会议名称";
		JsonObj={ok:false,errMsg:errMsg};
		sendjson(res,JsonObj);
	}else{
		meeting.createMeetingInfo(userName, req.body, function(jsonObj) {
			sendjson(res,jsonObj);
		});
	}
...
var app = connect()// 创建新的connect对象
  ...
  .use(MeetingRouter.post('/createMeetingInfo', createMeetingInfo))
